            ...
            {#each faqs as faq}
              <tbody class="bg-white divide-y divide-gray-200">
                <tr>
                  ...
                  <td
                    class="px-6 py-6 whitespace-nowrap text-sm font-medium text-gray-900 flex justify-center"
                  >
                    <!-- {faq.active ? '⚡️' : ''} -->
                    <ActiveToggle
                      data={faq}
                      on:toggleSave={handleSaveToggle}
...
<Modal {deleteModelOpen} on:cancel={() => (deleteModelOpen = false)} on:delete={handleDelete}>
  <div class="">
    ...
    <p class="mt-3 ml-2 text-sm text-gray-700">
      Currently Published?: {deleteFaq.active ? "Yes" : "No"}
    </p>
  </div>
</Modal>
